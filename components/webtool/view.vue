<template>
  <div class="mb-2 space-x-1.5">
    <button @click="clearTagsList" class="outline-dotted outline-offset-2 outline-primary rounded-sm text-sm px-3 mr-3 mb-3 font-mixed bg-primary text-primary hover:outline"> Clear </button>
  ...
  <ul class="h-[300px] overflow-auto space-y-0.5">
    <li class="flex flex-nowrap space-x-1.5 items-center overflow-x-scroll" v-for="(item, index) in getWebTools" v-motion-fade-visible>
      <span class="font-mixed text-sm outline-primary/50 outline bg-primary text-primary rounded-lg text-center min-w-7 mx-2"> {{ index+1 }} </span>
